...
export default function InstrumentView({
  ...
  if (!inst) {
    return <StubInstrument tk={tk} meta={meta} onGoHome={onGoHome}
      onRefresh={onRefresh} isRefreshing={isRefreshing} refreshError={refreshError} />
  }
...
/* ── Locked instrument (free tier, non-free ticker) ── */
...
/* ── Stub (no full data yet) — with refresh button ── */
  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--ln)', borderRadius: 'var(--r2)', padding: 32, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--mu)', lineHeight: 1.5, marginBottom: 20 }}>
        ...
        {onRefresh && (
          <div>
            <button
              ...
              onClick={() => onRefresh(tk)}
              disabled={isRefreshing}
              style={{
                background: isRefreshing ? 'var(--bg3)' : 'linear-gradient(135deg, var(--cy) 0%, #0e9488 100%)',
                color: isRefreshing ? 'var(--mu)' : '#fff',
                border: 'none', borderRadius: 'var(--r)', padding: '14px 32px',
                fontSize: 14, fontWeight: 700, cursor: isRefreshing ? 'wait' : 'pointer',
                display: 'inline-flex', alignItems: 'center', gap: 8,
                transition: 'all .2s', minWidth: 240,
            ...
            <div style={{ fontSize: 11, color: 'var(--mu)', marginTop: 8 }}>
              Full 10-factor analysis generated in real-time
            </div>
            {refreshError && (
              <div style={{ fontSize: 12, color: 'var(--rd)', marginTop: 8, padding: '8px 16px', background: 'rgba(239,91,91,.1)', borderRadius: 'var(--r)' }}>
                {refreshError}
              </div>
            )}
          </div>
    ...
    <div>
      ...
      <div className="inst-header" style={{ display: 'flex', alignItems: 'flex-start', gap: 24, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
          ...
          {onRefresh && (
            <button
              onClick={() => onRefresh(inst.tk)}
              disabled={isRefreshing}
              style={{
                background: isRefreshing ? 'var(--bg3)' : 'var(--bg2)',
                border: '1px solid var(--ln)', borderRadius: 'var(--r)',
                padding: '6px 14px', fontSize: 11, fontWeight: 600,
                color: isRefreshing ? 'var(--mu)' : 'var(--cy)',
            ...
            >
              {isRefreshing ? (
                <>
                  <span className="refresh-spinner-sm" />
                  Refreshing...
                </>
              ) : (
                <>
          ...
          {lastRefresh && (
            <div style={{ fontSize: 9, color: 'var(--mu)' }}>
              AI-generated &middot; {new Date(lastRefresh).toLocaleTimeString()}
            </div>
          )}
          {refreshError && (
            <div style={{ fontSize: 10, color: 'var(--rd)' }}>{refreshError}</div>
          )}
        </div>
      </div>
      ...
      {view === 'analysis' ? (
        <AnalysisTab inst={inst} tier={tier} expandedFactors={expandedFactors} setExpandedFactors={setExpandedFactors} />
      ) : (
      ...
      <div className="action-bar">
        ...
        <button
          className={`btn-watch${watched ? ' watched' : ''}`}
          onClick={() => { toggleWatch(inst.tk); setWatched(!watched) }}
        >
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 28 }}>
      <div>
        ...
        {/* 10 Factors — top 3 expanded, rest collapsed */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700 }}>10-Factor Breakdown</h3>
          <button onClick={() => {
            const allOpen = inst.factors.every((_, i) => expandedFactors[i])
            const next: Record<number, boolean> = {}
            inst.factors.forEach((_, i) => { next[i] = !allOpen })
            setExpandedFactors(next)
          }} style={{ fontSize: 11, color: 'var(--cy)', fontWeight: 600 }}>
            {inst.factors.every((_, i) => expandedFactors[i]) ? 'Collapse all' : 'Expand all'}
          </button>
        </div>
        <div className="factor-grid">
          {inst.factors.map((f, i) => {
            ...
            return (
              <div key={i} className="factor-card" style={{ cursor: 'pointer' }}
                onClick={() => setExpandedFactors(prev => ({ ...prev, [i]: !prev[i] }))}>
                ...
                {open && (
                  <>
                    <div className="factor-desc" style={{ marginTop: 8 }}>{f.desc}</div>
